...
\DeclareMathOperator{\rad}{rad} % radical ideal, radical of power series, etc.
\DeclareMathOperator{\spec}{Spec} % spectrum of commutative ring.
\DeclareMathOperator{\maxspec}{MaxSpec} % spectrum of maximal ring.
...
\newcommand{\LL}{\mathbb{L}} % laplacian matrix
...
\DeclarePairedDelimiter{\bracInner}{\{}{\}} % auto resized curly brackets 
\NewDocumentCommand{\brac}{s m}{
	\IfBooleanTF{#1}
	{\bracInner{#2}} % normal sized if \bace*
	{\bracInner*{#2}} % auto resized if \bace
}
...
\newcommand{\vpl}{\varprojlim} % inverse limit
% \newcommand{\iff}{\Longleftrightarrow} % Make 'iff' arrow longer if desired

% leftsquigarrow
\newcommand{\leftsquigarrow}{\mathrel{\reflectbox{$\rightsquigarrow$}}}
