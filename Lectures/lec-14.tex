...
\begin{proof}
	...
	\]
	by the fact that \( (f) \subseteq M \implies f(x) \subseteq M \implies f(a) = 0\).
\end{proof}
...
\begin{conclusion}
	...
	\begin{itemize}
		...
		\item \( (x-a)(x-\overline{a}) = x^2 + \abs{a}^2 \) where \( a\in \CC\backslash \RR \).
	\end{itemize}
\end{conclusion}
...
\begin{lemma}
	...
	\begin{note}
		Note that here \( n \) can goes to \textbf{infinity}.
	\end{note}
\end{lemma}
...
\begin{remark}
	\leavevmode
	\begin{itemize}
		\item The proof basically implies that if \( a_1,\ldots, a_n \) are as above, and \( b\in \R \backslash
		      \{0\}\), then \( b \cdot \gcd(a_1,\ldots, a_n) \) is a \( \gcd(ba_1,\ldots, ba_n) \).
		\item If the \( n_{ij} \) above are all \( 0 \), then the gcd becomes a \textbf{unit}.
	\end{itemize}
\end{remark}
...
\begin{lemma}
	\label{lem:primitive-mult}
	If \( f, g \in R[x] \) are primitive, then \( f \cdot g \) is primitive.
\end{lemma}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item Suppose we write:
		      ...
		      \[
			      g = \underbrace{\frac{c(f)'}{c(f)}}_{=\frac{a}{b}} h \implies bg = ah
		      \]
		      Take the gcd of coefficients of \( bg \) and \( ah \) respectively, and using the fact
		      that \( g,h \) are primitive, see that \( b = a \cdot \)unit, thus \( \frac{c(f)'}{c(f)} \)
		      is a unit in \( R \).
	\end{enumerate}
\end{proof}
