...
\begin{remark}
  ...
  polynomial of \( a \implies \deg(f) = [L:K] \). If \( \sigma \in G(\qo{L}{K}) \implies \sigma(a) \) is a root 
...
\begin{eg}
  ...
  not lie in \( \RR \) see that it is only the identity map.
\end{eg}
...
\begin{proof}
  ...
  \begin{exercise}
    ...
    \begin{enumerate}
      ...
      \item If \( (K_i,\vp_i)_{i\in I} \) is a family of elements of \( \cP \), s.t. any two of them
        ...
        \] 
        \( \implies \) If \( \vp: \tilde{K} \to M \) is defined by \( \tilde{\vp}|_{K_i} = \vp_i \),
        then 
        \begin{itemize}
        ...
        \end{itemize}
      \item \( (K_i,\vp_i) \leq (\tilde{K},\vp_i) \; \forall \; i \).
    \end{enumerate}
  \end{exercise}
  ...
  \begin{proof}[\textbf{Proof of Claim}]
    ...
    polynomial of \( a \) over \( K_1 \), we have 
    \[
      \begin{aligned}
      ...
      \end{aligned}
    \] 
    since \( f\in K_1[x] \), \( L \) is algebraically closed, then there exists \( b \in M \), s.t.
...
\begin{corollary}
  ...
  \] 
  \begin{note} 
    In particular, this yields us the uniqueness of algebraic closure up to isomorphism.
  \end{note}
\end{corollary}
...
\begin{proposition}
  If \( R \) is a commutative ring of char \( p \), \( p \) prime integer. If 
  \[
    \begin{aligned}
      F: R & \to R \\ 
      F(u) &= u^p
