\begin{notation}
	...
	\[
		R^{(I)} \coloneqq \bigoplus_{i\in I}M_i \quad \textcolor{blue}{M_i = R} \; \forall \; i
	\]
	and basis being
	\[
		      ...
		      \[
			      \begin{aligned}
				      \im(f) & = \brac{\sum a_i u_i \; | \; a_i \in R, \text{ only finitely many } \ne 0} \\
				             ...
				             & = \textcolor{blue}{\sum_{i\in I} R u_i}
			      \end{aligned}
		      \]
...
\begin{itemize}
	...
	\item If \( R \) is not commutative, then it can happen that
	      \[
		      R \cong \; R \oplus R
	      \]
	      as left \( R \)-module. Namely it can happens that a free module might have two basis with
	      different cardinality. For example it has basis cardinality to be \( 1 \) or to be \( 2 \).
\end{itemize}
...
\begin{lemma}[\textbf{Nakayama's Lemma}]
	...
	maximal ideal is \( \underline{m} \). If \( M \) is a \textbf{finitely generated} \( R \)-module, such that
...
\[
	F(M) = \{0\} \implies M = \underline{m} M \implies M = \{0\}
\]
...
\begin{proof}[\textbf{Proof of Nakayama's Lemma \ref{lem:nakayama}}]
	...
	\implies \) by definition of the sum of modules:
	\[
	...
	contains all it's \textcolor{blue}{not invertible elements}, thus \( 1 + \alpha \) is invertible
	since we will have \( 1 + \alpha \not\in \underline{m} \) because \( 1 \) is invertible apparently.
	Thus
	\[
		u_i = (1+\alpha)^{-1} (1+\alpha) u_i = (1+\alpha)^{-1}\cdot 0 = 0\; \forall \; i
	\]
...
\begin{proof}
	...
	\[
		...
		\end{aligned}
	\]
	where \( \underline{m} M + N \) being the \textcolor{blue}{smallest submodule} containing \( \underline{m}M \) and
	\( N \).
\end{proof}
...
\begin{proposition}
	...
	\begin{enumerate}
		...
		\item Every non-empty family of submodules of \( M \) contains a \textcolor{blue}{maximal element} w.r.t.
		      inclusion.
...
\begin{note}
	Note that the maximal here is interesting, not necessarily meaning the maximum element. For
	example let \( R = \ZZ \) be a \( R \)-module, and we have \( \{2\ZZ, 3\ZZ\} \), in this case,
	both \( 2\ZZ \) and \( 3\ZZ \) are maximal elements.
\end{note}
