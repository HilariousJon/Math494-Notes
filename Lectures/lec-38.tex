...
\begin{proof}
	...
	\[
		...
		\Tr_{\qo{K'}{K}}(b_i)
	\]
	suppose \( f \) is the minimal polynomial of \( a \) over \( K \), then
	\[
		\begin{aligned}
			K'              & \cong \qo{K[x]}{(f)}                                                                 \\
			A_{\overline K} & = \overline{K} \otimes_K K' \cong \qo{\overline{K}[x]}{(f)}\text{\todo{check this!}}
		\end{aligned}
	\]
...
\begin{lemma}
	If \( K \hookrightarrow K' = K(a) \), where \( a^p \in K, a\not\in K \), then
	\[
