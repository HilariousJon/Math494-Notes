...
\begin{proof}
	...
	\begin{itemize}
		...
		\item \( \qo{M}{N} \) is Noetherian: Let
		      ...
		      (operation \( \pi \) here is order preserving bijection):
		      \[
	...
	\]
	Which can be \hl{reasoned as follows}:
	\[
		\begin{aligned}
...
\begin{corollary}
	...
	If \( M,N \) are left \( R \)-modules, then \( M\oplus N \) is Noetherian (Artinian) \hl{iff both} \(
...
\begin{note}
	It allow us to only need to check itself being Noetherian instead of \textcolor{red}{checking all its submodules}.
\end{note}
...
\begin{proof}
	...
	\[
		...
		\end{aligned}
	\]
	we can do this since \( R^n \) is a free module, and we only need to specify where its basis goes.
	...
	Noetherian. We also need the first isomorphism theorem here, so that we see:
	\[
		\qo{R^n}{\ker(\vp)} \cong  M
	\]
\end{proof}
...
\begin{claim}
	...
	\[
		\textcolor{blue}{p : N \rightarrow \qo{N}{f(M)}}
	\]
	is a cokernel of \( f \).
	\begin{itemize}
...
\begin{exercise}
	...
	then \textbf{t.f.a.e.}
	\begin{enumerate}
		\item \( \exists \) morphism \( p: M \to M' \), s.t. \( p\circ f = \Id_{M'} \).
