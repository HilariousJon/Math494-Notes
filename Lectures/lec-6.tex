...
or say:
\[
	S^{-1} R \coloneqq \{\frac{a}{s} \; | \; a\in R, s\in S\}
\]
...
See that it is well-defined: suppose \( \frac{a_1}{s_1} = \frac{b_1}{t_1} \) and \( \frac{a_2}{t_2}
= \frac{b_2}{t_2} \), we want to see:
\begin{equation}
	\label{eq:1}
...
\begin{eg}
	When there is a zero divisor in \( S \), consider the case \( R = \qo{\ZZ}{6\ZZ} \) with \( S = \{\overline{1}, \overline{2}, \overline{4} \} \subseteq R \), there is a ring isomorphism:
	\[
		S^{-1} R \cong \; \qo{\ZZ}{3\ZZ}
	\]
	which, as one can see, there could never be injective homomorphism from \( R \) to \( S^{-1}R \)
	as \( \abs{R} = 6 \) and \( \abs{S^{-1}R} = 3 \). But the point is, even though there is
	zero-divisor in \( S \), we can \textcolor{red}{still} define \( S^{-1}R \), which behaves in a
	somewhat \textcolor{blue}{peculiar way}.
\end{eg}
...
\begin{remark}
	If \( R \) is an integral domain and \( S = R \backslash \{0\} \), then \( S^{-1} R \) is a
	\textcolor{red}{field}.
\end{remark}

\begin{remark}
	\( S^{-1}R \) has a canonical structure of \( R \)-Algebra with the following canonical ring
	homomoprhism:
	\[
		\begin{aligned}
			\vp: R & \to S^{-1}R   \\
		...
		\end{aligned}
	\]
	Note that it is not injective in general, we care whether it is injective because we want not to
	\textcolor{blue}{lose information}.
\end{remark}

\begin{remark}
	\leavevmode
	\begin{enumerate}
		\item \( a\in \ker(\vp) \iff \frac{a}{1} = \frac{0}{1} \iff \exists \; s\in S \), s.t. \( sa = 0 \).
		      Hence: \(\vp \) is not injective if and only if \( \exists \; s\in S \), which is a \textbf{zero divisor}.
		\item \( S^{-1}R = \{0\} \) iff \( 0\in S \), it tells us in general we don't care about the
		      case where \( 0 \in S \).
	\end{enumerate}
\end{remark}

\begin{eg}[\textbf{Construction of Fraction Field}]
	Let \( R \) be a \textcolor{red}{integral domain} (no zero-divisor in \( R \)), and \( S = R \backslash \{0\} \), then \( \vp : R \to S^{-1} R \) is injective, see that \( S^{-1}R \) in this case is a field: it is not \( 0 \), it is commutative, and if \( \frac{a}{s} \ne 0 (\iff a\ne 0) \implies\) this has the multiplicative inverse \( \frac{s}{a} \) since:
	\[
	...
	\begin{enumerate}
		\item If \( R = \ZZ \rsa \QQ \).
		\item If \( F \) be a field, and \( R = F[x_1, \ldots, x_n] \rsa \) field of rational function
		      \( F(x_1, \ldots, x_n) \) which is quotients of polynomials.
	\end{enumerate}
	\begin{note}
...
\begin{eg}
	\leavevmode
	\begin{itemize}
		\item Let \( f \in R \) and \( S = \{1, f, f^2, \ldots\} = \{f^n \; | \; n \in \ZZ_{>0}\}\) be a multiplicative system, then \( S^{-1} R \) is denoted by \( R_f \).
		\item There is a \textbf{universal property} of \( S^{-1} R \): Suppose \( S \subseteq R \) is a multiplicative system and \( \vp: R \to S^{-1} R \) is the canonical ring homomorphism, then:
		      \begin{enumerate}
...
\begin{remark}
	The essence of localizing \( R \) into \( S^{-1} R \) is to find a
	\textcolor{red}{\textbf{smallest ring}}, such that the element of \( S \) in the ring will become
	\textbf{invertible} in the sense of canonical ring homomorphism. Thus such construction is unique
	up to the ring, and thus we have the above universal property.
\end{remark}
...
\begin{eg}
	If \( R = \ZZ \), then
	\begin{enumerate}
		\item \( \{0\} \) is a prime ideal (\( \ZZ \) itself is an intergal domain).
