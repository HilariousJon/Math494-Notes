...
\begin{eg}
	...
	\]
	which, as one can see, there could never be injective homomorphism from \( R \) to \( S^{-1}R \) as \( \abs{R} = 6 \) and \( \abs{S^{-1}R} = 3 \). But the point is, even though there is zero-divisor in \( S \), we can \textcolor{red}{still} define \( S^{-1}R \), which behaves in a somewhat peculiar way.
\end{eg}
...
\begin{remark}
	If \( R \) is an integral domain and \( S = R \backslash \{0\} \), then \( S^{-1} R \) is a field.
\end{remark}
