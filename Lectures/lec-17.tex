...
\[
	...
	= (a\star b) u = (ba) u
\]
thus the quality with the \textcolor{blue}{question mark} holds, which is what we intended.
...
\begin{eg}
	\leavevmode
	\begin{enumerate}
		\item If we consider \( R \) as a left/right \( R \)-module, it's submodules are \hl{left/right
			      ideals} of \( R \).
		\item If \( f: M \to M' \) is a morphism of left \( R \)-modules, the kernel is a \hl{submodule} of
		      \( M \):
		      \[
			      \ker(f) \coloneqq  \{u \in M \; | \; f(u) = 0\}
		      \]
		      since \( f(u)=0 \implies f(au)=af(u)=a0 = 0 \). Besides, the \hl{image is also a submodule} of
		      \( M' \), \( \im(f) \subseteq M' \), since \( af(u) = f(au) \).
		\item Trivial examples, Let \( R \) be ring:
		      \begin{itemize}
			      \item \( 0_R \)-module: \( \{0\} \).
			      \item \( M \) be any left \( R \)-module, have:
			            \[
				            \begin{aligned}
					            \{0\} & \subseteq M \\
					            M     & \subseteq M
...
\begin{remark}
	...
	\begin{conclusion}
		...
		\end{itemize}
		It is literally just the same thing, with notation has the same sense.
	\end{conclusion}
\end{remark}
