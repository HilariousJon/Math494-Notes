...
\begin{definition}[\textbf{Direct Product of \( R \)-modules}]
	...
	\]
	with \hl{component-wise scalar multiplication}:
	\[
...
\begin{definition}[\textbf{Direct Sum of \( R \)-modules}]
	...
	\]
	One can check that this is indeed a submodule. Namely there is only finitely many entry that is
	non zero.
\end{definition}

\begin{remark}
	In fact when \( I \) is finite, the notion of direct sum and direct product coincide, since the
	condition of being zero for all but finitely many entries is automatically satisfied.
\end{remark}
...
\begin{note}
	\textcolor{red}{If \( I \) is finite, then
		\[
