...
\begin{theorem}[\textbf{Universal Property of} \( \mathbf{R[x]} \)]
	For every ring homomorphism \( \vp: R \to S \) with \( R,S \) commutative and for every
	\( \mathbf{a\in S} \), there is a \textbf{unique} ring homomorphism \( \psi: R[x] \to S \), s.t.
	\begin{enumerate}
...
\begin{proof}
	...
	For existence, we use this formula to define \( \psi: R[x] \to S \) explicitly, thus property 1
	and 2 is clear, only left to check that \( \psi \) is actually a ring homomorphism:
	\begin{itemize}
...
One can iterate this since \( R[x] \) is still a commutative ring, and thus get multi-variable
polynomial ring over \( R \), which is defined recursively by:
\[
...
\begin{theorem}[\textbf{Universal Property of }\( \mathbf{R[x_1, \ldots, x_n]} \)]
	\( \forall \) ring homomorphism \( \vp: R\to S \), \( R,S \) commutative, and \( \forall \; a_1,
	\ldots, a_n \in S \), there exists a \textbf{unique} ring homomorphism \( \psi: R[x_1, \ldots, x_n] \to S \), s.t.
	\begin{enumerate}
		\item the following diagram is commutative:
		      \[
			      ...
			      \end{tikzcd}
		      \]
		\item \( \textcolor{red}{\psi(x_i) = a_i \; \forall i \in \br{1,n}} \).
	\end{enumerate}
\end{theorem}
...
The proof is straightforward by using induction on \( n \) with the previous universal property of
\( R[x] \).

\begin{eg}
	If \( \sigma \in S_n \implies \exists ! \) ring homomoprhism, s.t. the following diagram is
	commutative:
	\[
		\begin{tikzcd}
		...
		\end{tikzcd}
	\]
	and \( f_{\sigma}(x_i) = x_{\sigma(i)} \quad \forall i \). In fact this is a ring isomoprhism,
	thus be a automorphism, with inverse being \( f_{\sigma^{-1}} \). In particular it shows that the
	process of constructing \( R[x_1, \ldots, x_n] \) is just labelling anmd \textbf{doesn't matter
		with order} of \( x_1, \ldots, x_n \).
\end{eg}
...
\begin{definition}[\textbf{Ring of Formal Power Series}]
	...
	See that \( (R[[x]], +, \cdot) \) is again a \textbf{commutative ring}, s.t. we have
	\( R[x] \subseteq R[[x]] \) being a subring.
\end{definition}
...
\begin{definition}[\textbf{R-Algebra}]
	\label{def:r-algebra}
	Suppose that \( R \) is a commutative ring, an \underline{\( R \)-Algebra} is \textcolor{red}{a
		ring \( S \) together with a ring homomorphism} \( R \xrightarrow{\vp}S \),
	s.t. \( \vp(a) b = b\vp(a) \; \; \forall\;  a \in R, b\in S \).
\end{definition}
