\begin{proposition}
	There is no multiple root for any irreducible polynomial \( f \) on a ring of charatcertistic
	being \( 0 \).
\end{proposition}
...
\begin{proof}
	...
	\]
	with \( a_n \ne 0 \). Suppose that \( f \) has a multiple root, then it means that \( (x-a) \mid f \)
	and \( (x-a) \mid f' \), in particular, this means that \( \gcd(f, f') \ne 1 \). But then since \( f \) is irreducible,
	the only possiblity is then that \( f \mid f' \) with \( f \ne\sim 1 \). But then look at
	\[
		f' = n a_n x^{n-1} + \cdots + 2 a_2 x + a_1
	\]
	which attains \( \deg(f') < \deg(f) \), then the only possibility is that \( f'= 0 \) so that one
	can have \( f\mid f' \). But to make it a zero polynomial, we shall need \( n \cdot a_n = 0 \),
	but which is never happen in a field of characteristic \( 0 \), then this means that we cannot
	have \( f' = 0 \), which is then means \( \gcd(f,f') = 1 \), which then means there is no multiple
	root for \( f \).
\end{proof}

\begin{conclusion}
	For a field \( K \) of characteristic \( 0 \), any element within is separarble over \( K \).
\end{conclusion}
...
\begin{eg}
	...
	\begin{conclusion}
		...
		= px^{p-1} = 0 \cdot x^{p-1}= 0\), and thus \( b \) is not a simple root.
	\end{conclusion}
\end{eg}

\begin{eg}[\textbf{Explicit Example}]
	\label{eg:explicit-example}
	Let \( k \) be any field with characteristic \( p > 0 \), let \( K = k(x) \), \textbf{see that} \( x \in
	k(x)\smallsetminus k(x)^p \): if \( x = (\frac{f}{g})^p \; f,g \in k[x] \), then \( xg^p = f^p \), thus
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item Suppose now \( \text{char}(K) = p \), if \( K \ne K^p \), by taking \( a\in K
		      ...
		      extension, suppose there exists \( a\in L \), with minimal polynomial \( f \in K[x^p] \) (this
		      is the case where every root of \( f \) are \textcolor{blue}{multiple roots}):
		      \[
		      ...
		      \[
			      \begin{aligned}
				      ...
				      \text{by } f & = b_0^p + b_1^p x^p + b_2^p x^{2p} + \cdots + b_n^p x^{np}                        \\
				                   & = (b_0 + b_1 x + b_2 x^2 + \cdots + b_n x^n)^p \text{ by frobenius homomorphism.} \\
				                   & = g^p
			      \end{aligned}
		      \]
		      by using the fact that \( u \to u^p \) is ring homomorphism in a commutative ring of char \( p \). Since \( p > 1 \), this leads to
		      the contradiction with the \textcolor{blue}{irreducibility of \( f \)} \( \lightning \).
	\end{itemize}
\end{proof}
...
\begin{eg}
	...
	\begin{itemize}
		\item If \( K \) is algebraically closed, then \( K \) is perfect (no non-trivial algebraic extension.)
		      \begin{proof}
			      For field of char \( 0 \) things are proved. For field of char \( p > 0 \), for any element
			      \( a \in K \), can construct a polynomial \( f = x^p - a \) with root \( b^p = a \) since
			      algebraically closed, and thus see that \( K = K^p \) \( \implies \) Q.E.D.
		      \end{proof}
		\item If \( K \) is a finite field, then \( K \) is perfect.
		      \begin{proof}
			      ...
			      \textcolor{blue}{proposition above}, \( K \) is perfect.
		      \end{proof}
		\item If \( k \) is any field with char \( p > 0 \), then \( k(x) \ne  \) perfect. By
		      \textbf{Example} \ref{eg:explicit-example}, \( x \in k(x) \smallsetminus k(x)^p \), thus \( k \ne k^p\).
	\end{itemize}
\end{eg}
...
\begin{theorem}[\textbf{Primitive Element Theorem}]
	\label{thm:primitive-element-theorem}
...
\begin{lemma}[\textbf{From Homework}]
	Let \( R \) be an integral domain, and suppose \( G \) is a finite subgroup of the group \( R^\times \) of units of \( R \),
	then \( G \) is cyclic.
\end{lemma}

\begin{lemma}
	Let \( K \) be a field, and let \( f \) be some irreducible polynomial in \( K[x] \), let \( \alpha \) be any root of \( f \), then we have
	\[
		K(\alpha) \cong \qo{K[x]}{(f)}
	\]
\end{lemma}
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item Suppose \( K \) is infinite, and \( \qo{L}{K} \) is finite, then \( L = K(a_1,\ldots, a_n) \). We shall argue
		      ...
		      \[
			      K(a_1,\ldots, a_{n-1}) = K(b) \text{ \textcolor{blue}{by IH} } \implies L = K(b,a_n)
		      \]
		      ...
		      of \( b,c \) respectively \textcolor{red}{over \( K(a) \)}. The roots are simple since \( \qo{L}{K} \) is separable. Let
		      \begin{align*}
			      \text{roots of }f & : \beta_1=b, \beta_2,\ldots, \beta_m                                      \\
		      ...
		      \[
			      ...
			      \end{tikzcd}
		      \]
		      where the isomorphism is given by \( \gamma_1 \to \gamma_2 \). there exists morphism of extension of \( K(a) \)
		      \[
			      \begin{aligned}
		      ...
		      \begin{note}
			      ...
			      \[
				      \begin{aligned}
					      \sigma|_{K(a)}                         & = \Id                                                               \\
					      \implies \sigma(b + \lambda c)         & =
					      b + \lambda  c                                                                                               \\
					      \implies \sigma(b) + \lambda \sigma(c) & = \beta_1 + \lambda \gamma_1                                        \\
					      \implies \beta_i + \lambda \gamma_2    & = \beta_1 + \lambda \gamma_1                                        \\
