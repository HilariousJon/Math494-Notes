...
\begin{proof}[\textbf{Proof of Uniqueness Cont.}]
	...
	\begin{proof}
		Let \( n = \rank(M) \), give two such decomposition, for each prime \( frak p = (\pi) \) by taking
		...
		\[
			...
			\end{aligned}
		\]
		and by induction hypothesis we have \( n_i = n_i' \; \forall \; i \geq 2 \). Also,
		\textbf{Isomorphism} \ref{eq:pid-module-1} gives an isomorphism that
		\[
			\begin{aligned}
				\{\text{elem. annihilated by $\pi$ on LHS}\} & \cong \{\text{elem. annihilated by $\pi$ on RHS}\}                \\
				\implies \oplus_{i\geq 1}\underbrace{\bace{\qo{(\pi^{i-1})}{(\pi^i)}}^{n_i}}_{\cong
				\qo{R}{(\pi)}}                               & \cong \bigoplus_{i\geq 1} \bace{\qo{(\pi^{i-1})}{(\pi^i)}}^{n_i'}
			\end{aligned}
		\]
	...
	\begin{note}
		...
		determined by the \( n_{ij} \), for example:
		\begin{eg}
			\leavevmode
			\begin{align*}
