...
\begin{remark}
	...
	\begin{enumerate}
		...
		\item Since \( \ZZ[\sqrt{d}] \subseteq \QQ[\sqrt{d}] \), by the universal property of fraction field \( \KK \) of \( \ZZ[\sqrt{d}] \) gives us: \(f: \KK \to \QQ[\sqrt{d}] \) as a ring homomorphism.
		      ...
		      \end{itemize}
		      In particular this tells us \textbf{up to isomorphism}, the fraction field of
		      \( \ZZ[\sqrt{d}] \) is \( \QQ[\sqrt{d}] \), thus every element in \( \ZZ[\sqrt{d}] \) is
		      invertible in \( \QQ[\sqrt d] \).
		\item Define:
		      \[
			      \begin{aligned}
		...
		\item Define:
		      ...
		      \begin{note}
			      ...
			      \]
			      since \( \vp(uv) = \vp(u)\vp(v) \). But similar result doesn't holds for addition. Thus it is not a ring homomorphism.
		      \end{note}
		\item One can view \( \ZZ[\sqrt{d}] \) as being the the quotient ring \( \qo{\ZZ[x]}{(x^2 - d)} \). Since \( \ZZ \) is a Noetherian ring, by \textbf{Hilbert Basis Theorem} \ref{thm:hilbert-basis}, \( \ZZ[x] \) is also a Noetherian ring, and thus \( \ZZ[\sqrt{d}] \) is a \textbf{Noetherian ring}. Alternatively, one can see that \( \ZZ[\sqrt{d}] = (1, \sqrt{d}) \), so it's finitely generated, and being a Noetherian ring.
	\end{enumerate}
	\begin{eg}
		\label{eg:unit-condition}
