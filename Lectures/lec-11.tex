...
\begin{remark}
	...
	\begin{enumerate}
		...
		\item Since \( \ZZ[\sqrt{d}] \subseteq \QQ[\sqrt{d}] \), by the universal property of fraction field \( \KK \) of \( \ZZ[\sqrt{d}] \) gives us: \(f: \KK \to \QQ[\sqrt{d}] \) as a ring homomorphism.
		      \begin{itemize}
			      \item This is injective since \( \KK \) is a field, since clearly the map is induced by \( \ZZ[\sqrt{d}] \subseteq \QQ[\sqrt{d}]\) so it is not a zero map. The field has no non-trivial ideals, thus the only case would be \( \ker(f) = \{0\} \), yields injectivity.
