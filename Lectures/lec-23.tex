\begin{theorem}
	\label{thm:submodule-of-free-module-over-pid}
	...
	a_m w_m \) is a basis of \( G \). In particular, \( G \) is free.
\end{theorem}
...
\begin{proof}
	...
	\begin{enumerate}
		\item \( m = 0 \), this means that \( \forall \; u \in G, \frac{u}{1} = 0 \) in \( S^{-1} G \),
		      ...
		      with \( au = 0 \implies \)
		      \[
			      \sum_{i}(a\alpha_i) u_i = 0 \implies a\alpha_i = 0 \; \forall \; i \text{ by linearly
				      independent}
		      \]
		      since \( a \ne 0 \), thus by domain we have \( \alpha_i = 0\; \forall \; i \implies u=0
		      \). Hence \( G = \{0\} \), and the conclusion is clear.
		\item Inductive step: Assume that \( m > 0 \implies G \ne \{0\} \). Consider the \( R
		      ...
		      \( \vp \) such that \( \vp(G) \) is \textcolor{red}{maximal} among all such choices of \( \vp \) w.r.t.
		      inclusion. \( R \) is a PID implies that \( \vp(G) = (a_1) \) for some \( a_1 \in R \).
		      Can choose \( x_1 \in G \), s.t. \( \vp(x_1) = a_1 \).

		      \begin{intuition}
			      The ideal is that in a PID, the bigger the ideal, the smaller set of prime element the
			      generators have, so finding the maximal ideal is literally finding that element who has
			      least reducible elements from \( G \) to \( R \).
		      \end{intuition}
		      ...
		      \begin{proof}
			      ...
			      i \leq n\). (Here we shall need the universal property of direct product) In particular, have
			      \[
				      \begin{aligned}
					      \pi_j : F  & \to R \\
					      \pi_j(u_i) & =
				      ...
				      \end{aligned}
			      \]
			      if \( a_1 = 0 \), by \textcolor{blue}{maximality} of \( a_1 \implies \psi(G) = \{0\} \; \forall \; \psi :
		      ...
		      \end{proof}
		      \begin{claim}
			      If \( \psi: F \to R \) arbitrarily linear map, then \( a_1 \mid \psi(x_1) \) where \(
			      \vp(x_1) = a_1, x_1 \in G \).
		      \end{claim}

		      \begin{intuition}
			      As the above note, the elements we chose, the \( a_1 \) shall have the least number of
			      irreducible factors.
		      \end{intuition}

		      \begin{proof}
			      Let \( b = \gcd\{a_1,\psi(x_1)\}  \xRightarrow{ R \text{ PID}} (b) = (a_1, \psi(x_1)) \implies b = \alpha \underbrace{a_1}_{\vp(x_1)} + \beta
			      \psi(x_1)\) for some \( \alpha, \beta \in R \). Now let \( \vp' = \alpha \vp + \beta \psi
			      \implies \vp'(x_1) = b\) and with \( b \mid a_1 \implies (a_1) \subseteq (b)
			      \). Since \( x_1 \in G, b = \vp'(x_1) \implies (b) \subseteq \vp'(G) \implies \vp(G) = (a_1) \subseteq (b) \subseteq \vp'(G) \). By maximality: \( (\vp'(x))  = (\vp(x)) = (b) \implies b \sim a_1\) but \( b \mid
			      \psi(x_1) \implies a_1 \mid \psi(x_1)\).
		      \end{proof}
		      Now apply this for \( \psi = \pi_j \) for each \( j \), we have
		      \[
			      \pi_j (x_1) = a_1 c_j \text{ for some } c_j \in R \; \forall \; j
		      \]
		      and thus
		      \[
			      x_1 = \sum_{j=1}^n \pi_j (x_1) u_j = a_1 \cdot \underbrace{\sum_{j} c_j u_j}_{e_1}
			      \implies x_1 = a_1 e_1 \text{ for some } e_1 \in F
		      \]
		      and thus
		      \[
			      a_1 = \vp(x_1) = \vp(a_1 e_1) = a_1 \vp(e_1)
		      \]
		      ...
		      \begin{proof}
			      ...
			      \begin{itemize}
				      ...
				      \item \( K + Re_1 = F \): if \( u \in F \implies u - \vp(u)e_1 \in K \) since \(
				            \vp(u - \vp(u) e_1) = \vp(u) - \vp(u)\vp(e_1) = 0 \; (\vp(e_1) = 1) \implies F = K + Re_1 \).
			      \end{itemize}
		      \end{proof}
		      ...
		      \begin{proof}
			      ...
			      \begin{itemize}
				      \item \( (K \cap G) \cap R a_1 e_1 = \{0\} \) is clear by previous reasoning, basically
				            \textcolor{red}{larger} thing already intersect with only \( 0 \) \( (K \cap G \subseteq K, Ra_1
				            e_1 \subseteq R e_1) \).
				      \item \( G = (K \cap G) + R a_1 e_1 \): have \( \vp(G) = (a_1) \), if \( u \in G \implies  \)
				            \[
					            u - \underbrace{\underbrace{\vp(u)}_{\in (a_1)}e_1}_{\in R a_1 e_1} \in K \cap G
				            \]
				            since \( u \in G, a_1 e_1 \in G, u - \vp(u)e_1 \in K \) as above.
			      \end{itemize}
		      ...
		      \begin{conclusion}
			      ...
			      \]
		      \end{conclusion}
		      We have that \( a_1 \ne 0, x_1 = a_1 e_1 \implies e_1 \ne 0 \implies \rank(Ra_1 e_1) \ne
		      0\) and actually \( \rank(R a_1 e_1 ) = 1 \). Thus we have
		      \[
			      \rank(K \cap G) = \rank(G) - 1 = m - 1
		      \]
		      Apply induction on the assertion that ``\textcolor{blue}{every submodule of a free finitely generated
		      ...
		      \]
		      Let \( d = \gcd(a_1,a_2) \xRightarrow{R \text{ PID}} d = \gamma a_1 + \delta a_2\) for
		      some \( \gamma, \delta \in R \). If \(
		      \psi: F \to R\) is s.t.
		      \[
			      \begin{cases}
				      \psi(e_1) = \gamma \\
		      ...
		      \begin{note}
			      The definition of \( \psi \) here is legal as \( e_1,\ldots, e_, \) is basis of \( F \).
		      \end{note}
...
a_m e_m \), but it is
good enough for our purpose. A more general theorem of this version will be proved in
homework.
