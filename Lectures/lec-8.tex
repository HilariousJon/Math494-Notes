\begin{eg}
	...
	\begin{enumerate}
		...
		\item Let \( R \) be a commutative ring, \( P\subseteq R \) be a prime ideal, define \( S = R - P \), and thus be a multiplicative system. Define \( R_p := S^{-1} R \). By HW \#3:
		      ...
		      \]
		      since \( P \) is the largest prime ideal for \( S^{-1} R \) with \( S = R - P \), s.t. \(
		      S \cap P = \emptyset\).
		      \begin{eg}
	...
	\begin{enumerate}
		...
		\item If each \( I_\alpha \) is a radical ideal, then \( \bigcap_{\alpha}I_\alpha \) is also a radical ideal. The proof is quite straightforward.
		      \begin{note}
			      ...
			      However, if the family is a \textbf{totally ordered set characterized by reverse set
				      inclusion}, then this statement is true. See homework related to Zorn's Lemma, this also
			      gives us the statement that \textbf{every prime ideals have a minimal prime ideal}.
		      \end{note}
	\end{enumerate}
\end{remark}
...
\begin{corollary}
	In a commutative ring \( R \), since all prime ideal contains \( 0 \), and thus contain \( (0) \), we have:
	\[
		\bigcap_{\mathfrak p \in \spec R} \mathfrak p = \rad((0)) = \{x\in R\; | \; \exists \; n \in \ZZ_{>0}, \; x^n = 0\} =: \sqrt{(0)}
	\]
	In particular, A ring \( R \) is reduced if and only if
	\[
		\bigcap_{\mathfrak p \in \spec R} \mathfrak p = \{0\}
	\]
\end{corollary}
