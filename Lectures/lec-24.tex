...
\begin{proposition}
	...
	To give an \( R=\KK[x] \)-module \( M \) is equivalent to giving a \( \KK \)-vector space \( V \) and a
	\( \KK \)-linear map \( T: V \to V \).
\end{proposition}

\begin{proof}
	If \( M \) is an \( R = \KK[x] \)-module, via the inclusion, we have \( \KK \subseteq \KK[x] \), we get an
...
\begin{note}
	...
	\begin{itemize}
		...
		\item If we assume the \( f_i \) are monic, then they are \underline{unique}, monic means
		      ...
		      \]
		\item Notice that by the division algorithm, for \( \qo{\KK[x]}{(f_i)} \), we can write any
		      polynomial by \( a = q f_i + r \) with \( \deg(r) < \deg(f_i) \), then see that any
		      element in the quotient space can be written uniquely by
		      \[
			      \{1,x,x^2,\ldots,x^{\deg(f_i) - 1}\} \implies \dim_{\KK}\bace{\qo{\KK[x]}{(f_i)}} = \deg(f_i)
		      \]
	\end{itemize}
\end{note}
