...
\begin{theorem}
	...
	\end{enumerate}
	Hence this gives complete description of the classification of finite fields.
\end{theorem}
...
\begin{proof}
	...
	\[
		\begin{tikzcd}
			\FF_p \arrow[r, hook] \arrow[rd, hook]& L \arrow[dashed, rd, "\vp" ]  \\
...
\begin{definition}[\textbf{Splitting Fields}]
	...
	\]
	\begin{note}
		Note that there can be some \( \alpha_i = \alpha_j, \; i \ne j \).
	\end{note}
\end{definition}
...
\begin{proposition}
	...
	\end{enumerate}
	In particular, this shows that the splitting field of a given polynomial \( f \) is unique up to isomorphism.
\end{proposition}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item Suppose \( L \) is another splitting field of \( f \) over \( K \), we have
		      ...
		      \[
			      \begin{tikzcd}
				      K \arrow[r,hook] \arrow[rd,hook ] & L = K(\beta_1,\ldots, \beta_n) \arrow[dashed, rd, "\vp" ] & \\
...
derivative of \( f \). \( f \) is the minimal polynomial meaning that \( f \) is \textcolor{red}{irreducible} in \(
...
\begin{conclusion}
	For an \textbf{algebraic element} in a field extension, its minimal polynomial either has all roots to be
	multiple roots or has all roots to be simple roots.
\end{conclusion}

\begin{note}
	Note that here it is not necesssary to discuss the case where \( a \) is not algebraic, since if
	it is transcendental, it doesn't even attend a minimal polynomial, not to mention discuss whether
	it is a simple root or multiple root.
\end{note}

\begin{definition}[\textbf{Separable}]
	\leavevmode
	\begin{enumerate}
		\item Let \( a\in L \) be algebraic elements. \( a\in L \) is \underline{separable} over \( K \) if \( a \) is a simple root of
