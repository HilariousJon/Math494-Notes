...
\begin{definition}[\textbf{Noetherian Rings and Artinian Rings}]
	...
	\]
	\( R \) is \underline{left/right Artinian ring} if there exists \textbf{no} strictly decreasing sequence
	\[
...
\begin{note}
	Aritinian ring is a stronger property, most of the rings doesn't satisfy the property of artinian
	ring.
\end{note}
...
\begin{eg}
	\leavevmode
	\begin{enumerate}
		\item A field is both Noetherian and Artinian.
		\item Every finite ring is both Noetherian and ArtiNian.
		\item \( \ZZ \) is not a Artinian ring, because there exists the following strictly decreasing ideal sequence:
		      \[
			      2\ZZ \supsetneq 2^2 \ZZ \supsetneq 2^3 \ZZ \supsetneq \cdots
		      \]
		\item \( \ZZ \) is a Noetherian ring. One can directly use \( 1 \) to finitely generate it. Notably, one may use the following as construction for counterexample:
		      \[
			      (2) \subseteq (2,3) \subseteq (2,3,5) \subseteq (2,3,5,7) \subseteq \cdots
		      \]
		      But actually by \textbf{Bezout's theorem}, \( 2x + 3y = 1 \) and thus see that \( 1 \in (2,3) \implies (2,3) = \ZZ \).
	\end{enumerate}
\end{eg}
...
\begin{proposition}
	...
	\begin{enumerate}
		...
		\item \textcolor{red}{Every} non-empty family \( \cI \) of left/right ideals of \( R \) contains a maximal element. The maximal element namely is:
		      \[
			      \exists \; I \in \cI \; \text{s.t. if } I \subseteq J, \; J \in \cI \implies I=J
		      \]
		\item \textcolor{red}{Every} left/right ideal of \( R \) is finitely generated. Namely:
		      \[
...
\begin{proof}
	...
	\end{itemize}
\end{proof}
The following theorem gives us a basic idea that in fact a lot of rings a actually Noetherian,
because the property can inherit from some other ring. So we can build up Noetherian ring from the
existing one.
...
\begin{remark}
	Let \( R \) be a ring.
	\begin{enumerate}
		\item If \( I \) is a two-sided ideal in \( R \) and \( R \) is left (right) Noetherian, then
		      \( \qo{R}{I} \) is left (right) Noetherian.
		\item If \( R \) is commutative and Noetherian, then for every multiplicative system
		      \( S \subseteq R \), the ring fractions \( S^{-1} R \) is Noetherian.
	\end{enumerate}
\end{remark}
...
\begin{definition}[\textbf{Euclidean Domain}]
	A domain \( R \) is an \underline{Euclidean Domain} if there eixsts
	\( N: R\backslash\{0\} \to \ZZ_{\geq 0} \) being \textbf{arbitrary} functions, such that,
	\( \forall \; a,b \in R \) with \( b\ne 0 \), there exists \( q,r\in R \)
	(\textbf{not necessarily unique}), such that:
	\begin{itemize}
		\item \( a = bq + r \).
		\item Either \( r= 0 \) or (\( r\ne 0 \) and \( N(r)<N(b) \)). (the norm of \( r \) should be
		      smaller than the norm of \( b \))
	\end{itemize}
\end{definition}
...
\end{proposition}
In particular it tells us that every ideal in an Euclidean domain is principal. Thus it is a good theorem to tell that some integral domains are not Euclidean domains (with repsect to any norm \( N \)).
...
\begin{proof}
	Let \( (p) \) be a nonzero prime ideal in the PID \( R \) and let \( I = (m) \) be any ideal containing \( (p) \), we want to see that \( I = (p) \) or \( I = R \). Now \( p \in (m) \) thus \( p= rm \) for some \( r \in R \). Since \( (p) \) is a prime ideal and \( rm \in (p) \), either \( r\in (p) \) or \( m \in (p) \). If \( m\in (p) \) then \( (p) = (m) = I \). If \( r \in (p) \), write \( r = ps \) for some \( s\in R \). Then:
	\[
		p = rm = psm \implies sm = 1 \quad (R\text{ is a domain.})
	\]
	thus \( m \) is a unit so \( I = R \).
\end{proof}

\begin{corollary}
	If \( R \) be any commutative ring such that \( R[x] \) is a PID, then \( R \) is a field.
\end{corollary}

\begin{proof}
	\( (x) \) is maximal ideal, and see that:
	\[
		\qo{R[x]}{(x)} \cong \; R
	\]
\end{proof}
...
\begin{eg}
	...
	\begin{enumerate}
		...
		\item If \( \KK \) is a field, then \( \KK[x] \) is an Euclidean domain, with:
		      ...
		      \begin{eg}
			      Consider \( S = \frac{\qo{\ZZ}{3\ZZ}[x]}{(2x^3 + x + 1)} \), see that \( \# S = 27 = 3^2 \), which is determined by how many choices for \( r \).
		      \end{eg}
	\end{enumerate}
\end{eg}
