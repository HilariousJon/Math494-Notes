...
\[
	...
	\end{aligned}
\]
The map is simply given by the \hl{polynomial evaluation} at the point \( a \). We can easily check that
...
\begin{proof}
	...
	\]
	Note that here we have \hl{multiple variables} here, so we apply the algorithm for different variables
	one by one.
	this implies \( b = f(a) \). Hence \( f(a) = 0 \iff f \in (x_1 - a_1, \ldots, x_n - a_n) \).
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item To show that \( V(I(Z)) \subseteq \overline{Z} \), need to show that if \( Z\subseteq
		      ...
		      \subseteq I(Z) \implies  V(J) \supseteq V(I(Z)) \).
	\end{itemize}
\end{proof}
...
\begin{proof}
	...
	\) (like $\QQ$). This implies that \( \dim_{k} L \) is at most countable.
	...
	\begin{itemize}
		\item If \( \ker(\vp) = 0 \implies \) thus have the injection  \(\text{Frac(k[y])} \hookrightarrow L \). (It being a field,
		      ...
		      \end{proof}
		      \begin{note}
			      The injection here is important as it basically \hl{preserve the relation of dimensionality},
...
\begin{proof}[\textbf{Proof of Hilbert's Nullstellensatz
	...
	Let's take \( S = R[y] \supseteq J' \) where \( J' \) being the ideal generated by \( J \) and \( g \) where
	\[
		g = 1 - fy
	\]
	\begin{claim}
		\( V(J') = \emptyset \), it will has no Nullstellens.
	\end{claim}
	...
	\[
		...
		\end{aligned}
	\]
	See that the last term vanish with \( h(1-fy) = h(1-f\cdot \frac{1}{f}) = 0 \). In \( R_f \), have:
	\[
